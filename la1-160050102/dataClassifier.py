...
def enhancedFeatureExtractorDigit(datum):
    ...
    grid = numpy.ndarray(shape=(50,50), dtype=float)
    ...
    def getPerim():
        ...
        for i in range(1,49):
            for j in range(1,49):
                if(getPixelVal(i,j) != getPixelVal(i-1,j)):
                   grid[i][j] = 1
                if(getPixelVal(i,j-1) != getPixelVal(i,j)):
                   grid[i][j] = 1
        for i in range(50):
            for j in range(50):
                perimeter+=grid[i][j]
    ...
    def getCorners_oth():
        ...
        for i in range(1,49):
            for j in range(1,49):
                if(getPixelVal(i,j)):
                    if(corner_surr(i,j) <= 2 and corner_surr(i,j) >0):
                        # concave corners
                        corner += 1
        return corner
    ...
    def corner_surr(x,y):
        check = 0
        if(getPixelVal(x,y) == 1):
            for a in range(x-1,x+2):
                for b in range(y-1,y+2):
                    if(getPixelVal(a,b) == 0):
                        check += 1
        return check
    ...
    features[0] = getArea()
    features[1] = getPerim()
    features[2] = (getPerim()**2)/(getArea()*1.0)
    features[3] = getCorners()
    features[4] = getCorners_oth()
