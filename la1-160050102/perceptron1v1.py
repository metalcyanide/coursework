			...
			for i in range(len(trainingLabels)):
				for j in range(len(self.legalLabels)):
					if j!= trainingLabels[i]:
						if ((self.weights[frozenset([min(trainingLabels[i],j),max(trainingLabels[i],j)])]*trainingData[i]) > 0):
							if (trainingLabels[i] < j):
								self.weights[frozenset([trainingLabels[i],j])] -= trainingData[i]
						else :							
							if(trainingLabels[i] > j):
								self.weights[frozenset([j,trainingLabels[i]])] += trainingData[i]
