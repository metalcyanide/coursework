...
class PerceptronClassifier:
    ...
    def sample_data(self, trainingData, trainingLabels, sample_weights):
        ...
        data = []
        label = []
        index = util.nSample(sample_weights,np.arange(len(trainingData)),int(len(trainingData)*0.6))
        for i in index:
            data.append(trainingData[i])
            label.append(trainingLabels[i])
        return data,label
