...
def generate2optNeighbours(tour):
    ...
    for i in range(1,len(tour)-1):
    	for j in range(i+1,len(tour)):
    		if(i!=1 or j!=len(tour)-1):
    			all_possible_neighbours.append(tour[0:i]+tour[i:j+1][::-1]+tour[j+1:])
    ...
    for i in range(len(tour)):
        for j in range(i+2,len(tour)):
            for k in range(j+2,len(tour)):
                if((k+1 != i) or ((k+1 == len(tour)) and (i == len(tour)))):
                    a = tour[0:i+1]
                    brev = tour[i+1:j+1][::-1]
                    b1 = tour[i+1:j+1]
                    crev = tour[j+1:k+1][::-1]
                    c1 = tour[j+1:k+1]
                    d = tour[k+1:]
                    all_possible_neighbours.append(a+brev+crev+d)
                    all_possible_neighbours.append(a+c1+brev+d)
                    all_possible_neighbours.append(a+crev+b1+d)
                    all_possible_neighbours.append(a+c1+b1+d)
    "*** --------------  ***"
    return all_possible_neighbours    
...
def hillclimb_help(Neighbours):
	min_length = 98765432198.0
	min_tour = []
	for i in Neighbours:
		if(min_length > getTourLength(i)):
			min_length = getTourLength(i)
			min_tour = i
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    "*** YOUR CODE HERE ***"
    curr_length = getTourLength(initial_tour)
    curr_tour = initial_tour
    while(1):
    	next_length,next_tour = hillclimb_help(getNeighbours(curr_tour))
    	if(next_length > curr_length):
    		break
    	tourLengthList.append(next_length)
    	curr_tour = next_tour
...
def nearestNeighbourTour(initial_city):
    ...
    "*** YOUR CODE HERE ***"
    for itr in range(cities-1):
    	min_dist = 9876543212345.0
    	next_city = curr_city
    	for i in range(1,cities+1):
	    	if i not in tour:
	    		if(min_dist > getDistance(nodeDict[i],nodeDict[curr_city])):
	    			min_dist = getDistance(nodeDict[i],nodeDict[curr_city])
	    			next_city = i
    	curr_city = next_city
