...
class AdaBoostClassifier:
    ...
    def train( self, trainingData, trainingLabels):
        ...
        "*** YOUR CODE HERE ***"
        weights = [1 for i in range(len(trainingData))]
        weights = util.normalize(weights)
        for i in range(len(self.classifiers)):
            error = 0
            self.classifiers[i].train(trainingData,trainingLabels,weights)
            for j in range(len(trainingData)):
                classify_label = self.classifiers[i].classify([trainingData[j]])[0]
                if(classify_label!= trainingLabels[j]):
                    error += weights[j]
            for j in range(len(trainingData)):
                classify_label = self.classifiers[i].classify([trainingData[j]])[0]
                if(classify_label == trainingLabels[j]):
    ...
    def classify( self, data):
        ...
        for i in range(len(data)):
            ...
            for j in range(len(self.classifiers)):
                C= self.classifiers[j].classify([datum])
                classification += (C[0]) * self.alphas[j]
            res.append(np.sign(classification))
        return res
